...
export const CategoriesList = () => {
    ...
    return (
        <div className="max-w-[250px] max-h-[110vh] overflow-y-auto">
            {categorias &&
                <ul className={` bg-white rounded-md z-10 flex flex-col top-6  border border-Almost-transparent-black shadow-lg gap-3 `}>
                    ...
                    {categorias.length > 0 && categorias.map((categoria) => (
                        <Link key={categoria.id} to={`/categoria/${categoria.id}`}>
                            <li className={`hover:bg-slate-200 py-2 px-5 transition-colors uppercase ${categoryId == categoria.id?'bg-blue-200 hover:bg-blue-400' : ''}`}>
